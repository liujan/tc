	...
	*/
	...
	/*
		...
		@param
			...
			user_pwd
	*/
	public function register($data, $ext){
		if ($this->create($data)){ //判断数据是否符合要求

			$tranDb = new Model;
			$tranDb->startTrans();

			$path = '/rc1/Public/images/'.$data['user_name'].".".$ext;
			$photo['user_name'] = $data['user_name'];
			$photo['image_path'] = $path;
			$userPhoto = M("InfoPhoto");
			$result = $userPhoto->add($photo);
