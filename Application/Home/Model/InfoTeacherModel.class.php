...
class InfoTeacherModel extends Model{
	...
	*/
	...
	public function getTeacher($school_id, $college_id) {
		$teacher_list = strval($school_id).strval($college_id)."teacher_list";
		if (S($teacher_list)){
			$result = S($teacher_list);
		}
		else{
			$result = $this->where("school_id='$school_id' AND college_id='$college_id'")->field('teacher_id')->select();
			S($teacher_list, $result, array("type" => "Memcache"));
		}

		if(is_array($result)) {
			
			$teachers = array();
			foreach($result as $key1 => $record){
				foreach($record as $key2 => $id) {
	...
	public function getCourse($school_id, $college_id, $teacher_id) {
		$course_list = strval($school_id).strval($college_id).strval($teacher_id)."course_list";

		if (S($course_list))
			$result = S($course_list);
		else{
			$result = $this->where("school_id='$school_id' AND college_id='$college_id' AND teacher_id='$teacher_id'")->find();
			S($course_list, $result, array('type' => 'Memcache'));
		}
		if(is_array($result)) {
			$courses = array();
