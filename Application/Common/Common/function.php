...
function isLogin(){
	if ($_SESSION['user_name'] != null && $_SESSION['user_name'] != "" && $_SESSION['user_id'] != null && $_SESSION['user_id'] != "") {
		$user_name = $_SESSION['user_name'];
		$user_id = $_SESSION['user_id'];
		$user = M("InfoUser");
		$result = $user->where("user_name = '$user_name' AND $user_id='$user_id'")->find();
		if (!empty($result))
			return true;
		else
			return false;
...
function sendMail($to, $verify_code){
    ...
    $url = 'http://localhost/rc1/?m=Home&c=User&a=active&verify_code='.$verify_code.'<br/>如果以上链接无法点击，请将它复制到你的浏览器地址栏中进入访问，该链接24小时内有效。';
    $mail->Body = $url; //邮件内容
